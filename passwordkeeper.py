...
### \/ RUN THIS ONCE \/###
"""
conn = sqlite3.connect("C:\\Users\Asus\programming\python\passwordkeeper\Passwordkeeper.db")
bot = conn.cursor()
...
class getdb():
    ...
    def list(self):
        ...
        db = bot.fetchall()
        for row in db:
            ensite, enuser, enpassword, enkey = (
                row[0], row[1], row[2], row[3])
            site, user, password = (decrypt(ensite, enkey), decrypt(
                enuser, enkey), decrypt(enpassword, enkey))
...
def show():
    ...
    for y in llist:
        if len(y[0]) > len(biggestsite):
            biggestsite = y[0]
        if len(y[1]) > len(biggestuser):
            biggestuser = y[1]
    # define wich words need to get extra spaces to be the size of the others
    sitegroup = [x[0] for x in llist if len(biggestsite) > len(x[0])]
    usergroup = [x[1] for x in llist if len(biggestuser) > len(x[1])]
    ...
    newsitegroup = [site.strip() for site in sitegroup]
    newusergroup = [user.strip() for user in usergroup]
    ...
    for x, list in enumerate(tuple[0]):
        ...
        while len(nextuser) < len(biggestuser):
            nextuser += " "
        if lastsite != nextsite:
            print("\r")
        if nextsite.strip() == "site/platform":
            print(
                f"\n{Fore.GREEN}    {Fore.CYAN}      {nextsite}  --  {Style.RESET_ALL}{nextuser}  --  {Fore.RED}{nextpass}")
        else:
            print(
                f"{Fore.GREEN}     --> {Fore.CYAN} {nextsite}  --  {Style.RESET_ALL}{nextuser}  --  {Fore.RED}{nextpass}")
        lastsite = nextsite
...
def add():
    ...
    conn = sqlite3.connect("Passwordkeeper.db")
    bot = conn.cursor()
    insertform = (encrypt(site, key), encrypt(
        user, key), encrypt(password, key), key)
    bot.execute("INSERT INTO passwordkeeper VALUES(?,?,?,?)", insertform)
    conn.commit()
    conn.close()
...
def delete():
    ...
    for row in db:
        if (decrypt(row[0], row[3]) == site) and (decrypt(row[1], row[3]) == user) and (decrypt(row[2], row[3]) == password):
            ...
            bot.execute(
                "DELETE FROM passwordkeeper WHERE site=? AND user=? AND password=?", deleteform)
            conn.commit()
    conn.close()
...
def change():
    ...
    for row in db:
        if (decrypt(row[0], row[3]) == site) and (decrypt(row[1], row[3]) == user) and (decrypt(row[2], row[3]) == password):
            if param == "site":
                newsite = newparam, newuser, newpassword = (
                    encrypt(newparam, row[3]), row[1], row[2])
            elif param == "user":
                newsite, newuser, newpassword = (
                    row[0], encrypt(newparam, row[3]), row[2])
            elif param == "pass":
                newsite, newuser, newpassword = (
                    row[0], row[1], encrypt(newparam, row[3]))
            changeform = (newsite, newuser, newpassword,
                          row[0], row[1], row[2])
            bot.execute(
                "UPDATE passwordkeeper SET site=?, user=?, password=? WHERE site=? AND user=? AND password=?", changeform)
            conn.commit()
    conn.close()
    show()
...
def transfer():
    ...
    for x in getdb().all():
        data.append(
            {
                "site": decrypt(x[0], x[3]),
                "name": decrypt(x[1], x[3]),
                "pass": decrypt(x[2], x[3])
            }
        )
    import json
    dataJson.write(json.dumps(data, indent=4))
### \/ NOT DONE \/ ###
    ...
    while True:
        ...
        if inputs in ["help", "hel", "he", "h"]:
          ...
          {Fore.LIGHTBLACK_EX}clear      --> Clears the console{Style.RESET_ALL}
                    """)
        elif inputs in ["show", "sho", "sh", "s", "-s", "-show", "--show"]:
            if last_input in ["show", "sho", "sh", "s", "-s", "-show", "--show"]:
                os.system("cls")
                show()
            else:
                show()
        elif inputs in ["add", "ad", "a", "-a", "-add", "--add"]:
            add()
        elif inputs in ["delete", "delet", "dele", "del", "de", "d", "-delete", "--delete"]:
            delete()
        elif inputs in ["change", "chang", "c", "-c", "-change", "--change"]:
            change()
        elif inputs in ["search", "searc", "sear", "sea", "s", "-s", "-search", "--search"]:
            search()
        elif inputs in ["quit", "qui", "qu", "q", "exit", "ex", "gtfo", "getout", "gottagetout", "immaheadout", "bigbrain", "bigweiner"]:
            quit()
        elif inputs in ["clear", "clea", "cle", "cls"]:
            os.system("cls")
        elif inputs in ["show --raw"]:
            print(getdb().list())
        elif inputs in ["transfer --to json"]:
            transfer()
        else:
            first_time = True
        last_input = inputs
